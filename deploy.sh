...
DEFAULT_DB_USERNAME="modusponnens"
...
# If no password is entered, use a default value.
...
cd $PROJECT_PATH
terraform init
terraform plan -var-file="terraform.tfvars" -out plano
terraform apply -auto-approve plano